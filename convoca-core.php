...
add_action('bdv_log_cleanup', ['\Convoca\Core\Installer', 'run_cleanup']);

// Cron: log purge (60-day retention)
add_action('bdv_log_purge', ['\Convoca\Core\Installer', 'run_purge']);

// Cron: continue access code generation if interrupted during activation
add_action('bdv_continue_access_codes', function () {
...
function convoca_health_page(): void
{
    ...
    if (class_exists('\\Convoca\\Members\\Admin_Settings')) {
        try {
            ...
            $all_checks[] = ['title' => 'Biodevas Members', 'status' => 'error', 'message' => $e->getMessage()];
        }
    } else {
        $all_checks[] = ['title' => 'Biodevas Members', 'status' => 'warning', 'message' => __('Plugin no activo.', 'convoca-core')];
    }
    ...
    if (class_exists('\\Convoca\\Enroll\\Admin_Settings')) {
        try {
            ...
            $all_checks[] = ['title' => 'Biodevas Enroll', 'status' => 'error', 'message' => $e->getMessage()];
        }
    } else {
        $all_checks[] = ['title' => 'Biodevas Enroll', 'status' => 'warning', 'message' => __('Plugin no activo.', 'convoca-core')];
    }
    ...
    if (class_exists('\\Convoca\\Gateway\\Diagnostic')) {
        ...
        $all_checks[] = ['title' => 'Biodevas Gateway', 'status' => 'warning', 'message' => __('Plugin no activo.', 'convoca-core')];
    }
    ...
    if (function_exists('cst_get_system_checks')) {
        ...
        $all_checks[] = ['title' => 'Centro Social Turnos', 'status' => 'warning', 'message' => __('Plugin no activo o sin función de diagnóstico.', 'convoca-core')];
    }
...
function convoca_notifications_page(): void
{
    ...
    $all = get_user_meta(get_current_user_id(), Notifications::META_KEY, true) ?: [];
    ...
    <div class="wrap">
        ...
        <p><?php echo sprintf(esc_html__('Total: %d notificaciones', 'convoca-core'), $total); ?></p>
        <table class="wp-list-table widefat fixed striped">
            ...
            <tbody>
                ...
                <?php else: foreach ($items as $n):
                    $icon = match ($n['type'] ?? 'info') { 'success' => '✅', 'warning' => '⚠️', 'error' => '❌', default => 'ℹ️' };
                ?>
                <tr>
                    <td><?php echo $icon; ?></td>
...
{
    $cpts = ['miembro', 'actividad', 'inscripcion', 'pago', 'centro_turno', 'bdv_evaluacion', 'proyecto', 'registro_hora', 'bdv_documento'];
    foreach ($cpts as $cpt) {
...
function convoca_customize_new_menu(\WP_Admin_Bar $wp_admin_bar): void
{
    ...
    $remove_nodes = ['new-inscripcion', 'new-pago', 'new-bdv_evaluacion', 'new-bdv_documento'];
 ...
 */
add_action('admin_post_bdg_export_payments_pdf', function () {
    ...
    $posts = get_posts(['post_type' => 'pago', 'posts_per_page' => -1, 'post_status' => 'publish']);
    $headers = [__('Order ID', 'convoca-core'), __('Importe', 'convoca-core'), __('Estado', 'convoca-core'), __('Método', 'convoca-core'), __('Fecha', 'convoca-core')];
    $rows = [];
...
{
    if (!current_user_can('manage_options')) wp_die(__('No tienes permisos.', 'convoca-core'));
    ...
    $cards = [
        ['icon' => '👥', 'label' => __('Socios activos', 'convoca-core'), 'value' => $m['activos'], 'total' => $m['total'], 'url' => admin_url('admin.php?page=bdv-members')],
        ['icon' => '🆕', 'label' => __('Altas este mes', 'convoca-core'), 'value' => $m['nuevos_mes'], 'total' => '', 'url' => admin_url('admin.php?page=bdv-members')],
        ['icon' => '📝', 'label' => __('Inscripciones confirmadas', 'convoca-core'), 'value' => $i['confirmadas'], 'total' => $i['total'], 'url' => admin_url('admin.php?page=bde-inscripciones')],
        ['icon' => '⏳', 'label' => __('En lista de espera', 'convoca-core'), 'value' => $i['lista_espera'], 'total' => '', 'url' => admin_url('admin.php?page=bde-inscripciones')],
        ['icon' => '💰', 'label' => __('Ingresos este mes', 'convoca-core'), 'value' => $p['total_mes_fmt'], 'total' => '', 'url' => admin_url('edit.php?post_type=pago')],
        ['icon' => '🟡', 'label' => __('Turnos sin cubrir', 'convoca-core'), 'value' => $t['disponibles'], 'total' => $t['total'], 'url' => admin_url('edit.php?post_type=centro_turno')],
    ];
    ?>
    <div class="wrap">
        ...
        <div class="bdv-analytics-cards">
            <?php foreach ($cards as $card): ?>
                <a href="<?php echo esc_url($card['url']); ?>" class="bdv-analytics-card">
                    ...
                    <?php if ($card['total']): ?><div class="bdv-card-total">/ <?php echo esc_html($card['total']); ?> total</div><?php endif; ?>
        ...
        <div class="bdv-analytics-charts">
            <div class="bdv-chart-card">
                ...
                <?php echo \Convoca\Core\Admin_Analytics::render_chart('chartTrends', 'line', $trend['labels'], [
                    ['label' => 'Altas socios', 'data' => $trend['members'], 'color' => '#320028', 'border' => '#320028'],
                    ['label' => 'Inscripciones', 'data' => $trend['inscriptions'], 'color' => 'rgba(255,135,0,0.2)', 'border' => '#FF8700'],
                ]); ?>
            </div>
            <div class="bdv-chart-card">
                <h3>💰 <?php esc_html_e('Ingresos mensuales', 'convoca-core'); ?></h3>
                <?php echo \Convoca\Core\Admin_Analytics::render_chart('chartPayments', 'bar', $trend['labels'], [
                    ['label' => 'Ingresos (€)', 'data' => $trend['payments'], 'color' => '#059669', 'border' => '#059669'],
                ]); ?>
            </div>
            <div class="bdv-chart-card">
                <h3>📊 <?php esc_html_e('Inscripciones por estado', 'convoca-core'); ?></h3>
                <?php echo \Convoca\Core\Admin_Analytics::render_chart('chartInscriptionStates', 'doughnut',
                    ['Confirmadas', 'Pendientes', 'Espera', 'Canceladas'],
                    [['label' => 'Estado', 'data' => [$i['confirmadas'], $i['pendientes'], $i['lista_espera'], $i['canceladas']], 'color' => ['#059669','#f59e0b','#3b82f6','#ef4444']]]
                ); ?>
            </div>
            <div class="bdv-chart-card">
                <h3>💳 <?php esc_html_e('Métodos de pago (mes)', 'convoca-core'); ?></h3>
                <?php echo \Convoca\Core\Admin_Analytics::render_chart('chartPayMethods', 'doughnut',
                    ['Tarjeta', 'Bizum', 'Otros'],
                    [['label' => 'Método', 'data' => [$p['methods_pct']['tarjeta'], $p['methods_pct']['bizum'], $p['methods_pct']['otros'] ?? 0], 'color' => ['#0073aa','#46b450','#94a3b8']]]
                ); ?>
            </div>
            <div class="bdv-chart-card bdv-chart-card--wide">
                <h3>📋 <?php esc_html_e('Últimos 7 días — Pagos', 'convoca-core'); ?></h3>
                <?php echo \Convoca\Core\Admin_Analytics::render_chart('chart7days', 'bar', array_column($p['last_7_days'], 'label'), [
                    ['label' => 'Importe (€)', 'data' => array_map(fn($d) => round($d['total'] / 100, 2), $p['last_7_days']), 'color' => '#FF8700', 'border' => '#e67a00'],
                ], ['height' => '160px']); ?>
            </div>
        </div>
    </div>
...
function convoca_build_metrics(): array
{
    ...
    if (class_exists('\Convoca\Members\CPT_Miembro') && class_exists('\Convoca\Members\Estados')) {
        ...
        foreach (\Convoca\Members\Estados::LABELS as $slug => $label) {
            $counts[$slug] = (new \WP_Query([
                'post_type' => 'miembro', 'post_status' => 'publish',
                'posts_per_page' => 1, 'fields' => 'ids',
                'meta_query' => [['key' => '_bdv_estado_miembro', 'value' => $slug]],
            ]))->found_posts;
        }
    ...
    $metrics['members_new_month'] = (int) $wpdb->get_var($wpdb->prepare(
    ...
    if (class_exists('\Convoca\Enroll\CPT_Inscripcion')) {
        $pendientes_pago = new \WP_Query([
            'post_type' => 'inscripcion', 'post_status' => 'publish',
            'posts_per_page' => 1, 'fields' => 'ids',
            'meta_query' => [['key' => '_bde_estado', 'value' => 'pendiente_pago']],
        ]);
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}postmeta'")) {
        $metrics['payments_month'] = (float) $wpdb->get_var($wpdb->prepare(
    ...
    $metrics['turnos_sin_cubrir'] = (int) $wpdb->get_var($wpdb->prepare(
