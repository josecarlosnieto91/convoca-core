...
class Admin_Backup
{
    ...
    public function __construct()
    {
        ...
        add_action('admin_post_bdv_export_backup', [$this, 'handle_export']);
        add_action('admin_post_bdv_import_backup', [$this, 'handle_import_preview']);
        add_action('admin_post_bdv_import_backup_run', [$this, 'handle_import_run']);
    }

    public function register_page(): void
    {
        add_submenu_page('convoca-core', __('Copia de Seguridad', 'convoca-core'), __('Copia de Seguridad', 'convoca-core'), 'common_manage_backup', 'bdv-backup', [$this, 'render']);
    }

    public function render(): void
    {
        if (!current_user_can('common_manage_backup')) wp_die(__('No tienes permisos.', 'convoca-core'));
    ...
    {
        if (!wp_verify_nonce($_GET['_wpnonce'] ?? '', 'bdv_export_backup')) wp_die('Nonce inválido.');
        if (!current_user_can('common_manage_backup')) wp_die('No tienes permisos.');
        if (!class_exists('ZipArchive')) wp_die('ZipArchive no disponible.');
        ...
        if ($zip->open($tmp_file, \ZipArchive::CREATE | \ZipArchive::OVERWRITE) !== true) {
            $cleanup();
            wp_die('No se pudo crear el archivo temporal ZIP.');
        }
        ...
        // Members
        $export_all('miembro', ['ID', 'Nombre', 'Email', 'DNI', 'Plan', 'Estado', 'Vencimiento'], function ($id, $title) {
            return [$id, $title, get_post_meta($id, '_bdv_email', true), get_post_meta($id, '_bdv_dni', true),
                get_post_meta($id, '_bdv_plan', true), get_post_meta($id, '_bdv_estado_miembro', true), get_post_meta($id, '_bdv_vencimiento', true)];
        });

        // Inscriptions
        $export_all('inscripcion', ['ID', 'Nombre', 'Email', 'Estado', 'Actividad ID'], function ($id) {
            return [$id, get_post_meta($id, '_bde_nombre', true), get_post_meta($id, '_bde_email', true),
                get_post_meta($id, '_bde_estado', true), get_post_meta($id, '_bde_actividad_id', true)];
        });

        // Projects
        $export_all('proyecto', ['ID', 'Título', 'Inicio', 'Fin', 'Activo'], function ($id) {
            return [$id, get_the_title($id), get_post_meta($id, '_bdv_fecha_inicio', true),
                get_post_meta($id, '_bdv_fecha_fin', true), get_post_meta($id, '_bdv_activo', true)];
        });

        // Centro Social Turnos
        $export_all('centro_turno', ['ID', 'Título', 'Responsable', 'Estado', 'Hora Fin'], function ($id) {
            $resp_id = (int) get_post_meta($id, '_id_responsable', true);
            return [$id, get_the_title($id), $resp_id ?: '',
        ...
        $settings = [];
        $allowed_options = ['bdg_settings', 'bdv_members_settings', 'bdv_members_plans', 'bde_settings', 'cst_hora_apertura', 'cst_hora_cierre', 'cst_calendar_page_url'];
        ...
        if (!file_exists($tmp_file)) wp_die('Error fatal: ZIP no generado.');
    ...
    public function handle_import_preview(): void
    {
        ...
        if (empty($_FILES['backup_zip']['tmp_name'])) wp_die('No se ha subido ningún archivo.');
        $file = $_FILES['backup_zip'];

        if ($file['error'] !== UPLOAD_ERR_OK) wp_die('Error en la subida: ' . $file['error']);
        if (strtolower(pathinfo($file['name'], PATHINFO_EXTENSION)) !== 'zip') wp_die('Formato inválido. Solo ZIP.');
        if ($file['size'] > self::MAX_ZIP_SIZE) wp_die('Archivo demasiado grande.');

        $zip = new \ZipArchive();
        if ($zip->open($file['tmp_name']) !== true) wp_die('No se pudo abrir el ZIP.');

        // Robust validation
        $entities = [];
        $valid_entities = ['miembros', 'inscripciones', 'proyectos', 'turnos'];
        foreach ($valid_entities as $name) {
            if ($zip->locateName($name . '.csv') !== false) {
                $stream = $zip->getStream($name . '.csv');
                $count = 0;
                while (fgets($stream)) $count++;
                fclose($stream);
                $entities[$name] = ['label' => $this->entity_label($name), 'count' => max(0, $count - 1)];
            }
        }
        if ($zip->locateName('settings.json') !== false) {
            $entities['settings'] = ['label' => __('Configuración del Sistema', 'convoca-core'), 'count' => 1];
        }
        ...
        if (!move_uploaded_file($file['tmp_name'], $target)) wp_die('Error al guardar el temporal.');

        set_transient('bdv_import_preview_' . get_current_user_id(), [
            'token' => $token,
            'entities' => $entities,
        ], self::PREVIEW_TTL);
    ...
    public function handle_import_run(): void
    {
        ...
        $selected = isset($_POST['entities']) ? (array) $_POST['entities'] : [];
        ...
        foreach ($selected as $entity) {
            if (isset($cap_checks[$entity]) && !current_user_can($cap_checks[$entity])) {
                wp_die("No tienes permisos para importar $entity.");
            }
        }
        ...
        if (!file_exists($filepath)) wp_die('Archivo temporal no encontrado.');

        $zip = new \ZipArchive();
        if ($zip->open($filepath) !== true) wp_die('No se pudo procesar el ZIP.');
        ...
        try {
            $order = ['settings', 'proyectos', 'miembros', 'inscripciones', 'turnos'];
            
            foreach ($order as $entity) {
            if (!in_array($entity, $selected, true)) continue;
            ...
            $csv_name = $entity . '.csv';
            if ($zip->locateName($csv_name) === false) continue;

            $stream = $zip->getStream($csv_name);
            if (fread($stream, 3) !== chr(0xEF) . chr(0xBB) . chr(0xBF)) rewind($stream);

            $headers = fgetcsv($stream, 0, ',', '"');
            if (!$headers) { fclose($stream); continue; }

            $post_type = match($entity) {
                'miembros' => 'miembro', 'proyectos' => 'proyecto', 'inscripciones' => 'inscripcion', 'turnos' => 'centro_turno',
                default => false
            };
            if (!$post_type) { fclose($stream); continue; }
    ...
    private function import_settings($zip): void
    {
        ...
        $allowed = ['bdg_settings', 'bdv_members_settings', 'bdv_members_plans', 'bde_settings', 'cst_hora_apertura', 'cst_hora_cierre', 'cst_calendar_page_url'];
        ...
        foreach ($allowed as $key) {
            ...
            if ($current) update_option($key . '_backup_' . wp_date('Y-m-d-His'), $current, false);
