...
class Admin_Templates {
    ...
    private static function create_versioned_backup(): void
    {
        ...
        $backups = get_option(self::$option_name . '_versions', []);
    ...
    private static function sanitize_pdf_template($content) {
        ...
        $pdf_structural = [
            'html' => ['xmlns' => true, 'lang' => true, 'style' => true],
            'head' => [],
            'title' => [],
            ...
            'center' => [],
            'page_break' => ['style' => true], // Common custom tag/class
        ];

        // Ensure table attributes are allowed
        $pdf_structural['table'] = array_merge($allowed['table'] ?? [], [
            'cellspacing' => true, 'cellpadding' => true, 'border' => true, 'align' => true, 'width' => true, 'style' => true
        ]);
    ...
    public static function render_page() {
        ...
        <div class="wrap bdv-common-settings-wrap">
            ...
            <p><?php echo sprintf(esc_html__('Utiliza etiquetas como %s, %s, %s para que se rellenen automáticamente según el contexto.', 'convoca-core'), '<code>{{nombre}}</code>', '<code>{{dni}}</code>', '<code>{{fecha}}</code>'); ?></p>
