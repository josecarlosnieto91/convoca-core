...
class Admin_Setup_Wizard
{
    ...
    public function __construct()
    {
        ...
        add_action('admin_post_bdv_wizard_save', [$this, 'handle_save']);
        add_action('admin_post_bdv_wizard_skip', [$this, 'handle_skip']);
        add_action('admin_post_bdv_wizard_create_pages', [$this, 'handle_create_pages']);
        add_action('admin_post_bdv_wizard_complete', [$this, 'handle_complete']);
    }
    ...
    public function maybe_redirect(): void
    {
        ...
        if (get_option(self::COMPLETED_OPTION) || get_option(self::DISMISS_OPTION)) return;
        ...
        if (isset($_GET['page']) && $_GET['page'] === 'bdv-setup-wizard') return;

        // Session-level guard to avoid infinite annoyance
        $user_id = get_current_user_id();
        if (get_transient(self::SEEN_OPTION . '_' . $user_id)) return;
        ...
        if (!in_array($pagenow, ['index.php', 'plugins.php'], true)) return;
    ...
    private function is_config_complete(): bool
    {
        ...
        // 1. Tables
        foreach (['biodevas_logs', 'biodevas_locks'] as $t) {
            if ($wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}{$t}'") !== $wpdb->prefix . $t) return false;
        }

        // 2. Plugins
        if (!class_exists('\\Convoca\\Core\\Utils')) return false;

        // 3. Mandatory Pages
        foreach (['alta-socios', 'panel-socio', 'pago'] as $slug) {
            if (!get_page_by_path($slug)) return false;
        }
    ...
    {
        return ['label' => $label, 'ok' => $ok, 'msg' => $ok ? $ok_msg : $err_msg];
    }
    ...
    private function step_pages(): void
    {
        ...
        $pages = [
            'alta-socios' => ['title' => 'Alta de Socios', 'sc' => '[biodevas_alta]', 'req' => true],
            'panel-socio' => ['title' => 'Mi Panel de Socio', 'sc' => '[mi_panel]', 'req' => true],
            'pago'        => ['title' => 'Página de Pago', 'sc' => '[biodevas_pago]', 'req' => true],
        ];
    ...
    public function handle_create_pages(): void
    {
        ...
        try {
            $pages = [
                'alta-socios' => ['title' => 'Alta de Socios', 'sc' => '[biodevas_alta]'],
                'panel-socio' => ['title' => 'Mi Panel de Socio', 'sc' => '[mi_panel]'],
                'pago' => ['title' => 'Pago', 'sc' => '[biodevas_pago]'],
            ];
            ...
            foreach ($pages as $slug => $info) {
                if (!get_page_by_path($slug)) {
                    $page_id = wp_insert_post([
                        ...
                        'post_content' => $info['sc'], 'post_status' => 'publish', 'post_type' => 'page'
                    ]);
    ...
    {
        $plans = class_exists('\\Convoca\\Members\\CPT_Miembro') ? \Convoca\Members\CPT_Miembro::get_plans() : [];
        ?>
        ...
        <?php else: ?>
            ...
            <ul><?php foreach ($plans as $p) echo '<li>' . esc_html($p['label']) . '</li>'; ?></ul>
        <?php endif; ?>
    ...
    {
        return ['is_ready' => $this->is_config_complete()];
    }
    ...
    private function step_nav(int $current, bool $can_continue): void
    {
        ...
        if ($current > 1) echo '<a href="' . esc_url(admin_url('admin.php?page=bdv-setup-wizard&step=' . ($current - 1))) . '" class="biodevas-btn biodevas-btn-outline">Anterior</a>';
        else echo '<div></div>';
        if ($can_continue && $current < 6) echo '<a href="' . esc_url(admin_url('admin.php?page=bdv-setup-wizard&step=' . ($current + 1))) . '" class="biodevas-btn biodevas-btn-primary">Siguiente</a>';
        echo '</div>';
    }
}
