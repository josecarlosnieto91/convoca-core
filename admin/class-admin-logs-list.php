...
class Admin_Logs_List extends \WP_List_Table
{
    ...
    {
        if (!$item->user_id) return '<span style="color:#94a3b8;">—</span>';
    ...
    public function prepare_items(): void
    {
        ...
        if ($filter_context) { $where[] = 'context = %s'; $args[] = $filter_context; }
        if ($filter_level) { $where[] = 'level = %s'; $args[] = $filter_level; }
        if ($filter_date_from) { $where[] = 'created_at >= %s'; $args[] = $filter_date_from . ' 00:00:00'; }
        if ($filter_date_to) { $where[] = 'created_at <= %s'; $args[] = $filter_date_to . ' 23:59:59'; }
        if ($search) {
