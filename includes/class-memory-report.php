...
class Memory_Report
{
    ...
    public static function init(): void
    {
        ...
        add_action('bdv_weekly_event', [__CLASS__, 'auto_generate']);
        // Admin: manual generation
        add_action('admin_post_bdv_generate_memory', [__CLASS__, 'handle_admin_generate']);
    }
    ...
    public static function generate_pdf(int $timestamp): ?string
    {
        ...
        $altas_mes = (int) $wpdb->get_var($wpdb->prepare(
            ...
            wp_date('Y-m-d', $month_start), wp_date('Y-m-d', $month_end)
        ));
        $insc_mes = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->posts}
