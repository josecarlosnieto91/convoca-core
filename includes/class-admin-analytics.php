...
class Admin_Analytics
{
    ...
    public static function get_member_stats(): array
    {
        ...
        $vencen_7d = (int) $wpdb->get_var($wpdb->prepare(
    ...
    public static function get_payment_stats(): array
    {
        ...
        $default = [
            ...
            'cantidad_mes'  => 0,
            'hoy'           => 0,
            'methods'       => ['tarjeta' => 0, 'bizum' => 0],
            'methods_pct'   => ['tarjeta' => 0, 'bizum' => 0, 'otros' => 0],
            'last_7_days'   => [],
        ];
        ...
        $map = [];
        foreach ($dias as $r) {
            $map[$r->day] = ['count' => (int) $r->cnt, 'total' => (int) $r->total];
        }
    ...
    public static function get_trends(): array
    {
        ...
        foreach ($months as $m) {
            ...
            $members_trend[] = (int) $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->posts}
                 WHERE post_type = 'miembro' AND post_status = 'publish'
            ...
            $inscriptions_trend[] = (int) $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->posts}
            ...
            $payments_trend[] = (float) $wpdb->get_var($wpdb->prepare(
     ...
     */
    public static function render_chart(string $canvas_id, string $type, array $labels, array $datasets, array $opts = []): string
    {
        $json_data = [];
        ...
        $chart_opts = json_encode([
            ...
            'plugins' => [
                'legend' => ['display' => count($datasets) > 1 || $type === 'pie'],
            ],
