...
class Webhook_Manager
{
    ...
    public function __construct()
    {
        ...
        add_action('bdv_webhook_retry', [__CLASS__, 'process_retries']);
    }
    ...
    {
        $this->dispatch('payment.reminder_sent', [
            'member_id'    => $member_id,
    ...
    {
        $this->dispatch('enrollment.cancelled', [
            'inscripcion_id' => $inscripcion_id,
            'actividad_id'   => $actividad_id,
        ]);
    }
    ...
    {
        $this->dispatch('enrollment.checkin', [
            'inscripcion_id' => $inscripcion_id,
    ...
    {
        $this->dispatch('volunteer.hours_logged', [
    ...
    {
        $this->dispatch('volunteer.hours_approved', [
            'registro_id' => $registro_id,
            'member_id'   => $member_id,
            'horas'       => get_post_meta($registro_id, '_bdv_horas', true),
        ]);
    ...
    {
        $this->dispatch('volunteer.hours_rejected', [
            'registro_id' => $registro_id,
            'member_id'   => $member_id,
            'horas'       => get_post_meta($registro_id, '_bdv_horas', true),
        ]);
    ...
    public static function process_retries(): void
    {
        ...
        try {
            ...
            foreach ($retries as $retry) {
                ...
                if ($success) {
                    $wpdb->delete($table_name, ['id' => $retry->id]);
                } else {
    ...
    {
        $logs = get_option('bdv_webhook_log_' . $webhook_id, []);
